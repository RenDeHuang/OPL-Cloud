...
test("launch freeze fixes the V2 products, owner lanes, settlement, and verification slot", async () => {
  ...
  assert.deepEqual({
    ...
    requestHashReconciliation: {
      ...
      requiredLedger: "observed_provider_describe_cvm_tag_readback_provider_error_cvm_attempted_1_confirmed_0_unknown_1_missing_only_opl_account_id_node_zero",
      persistence: "versioned_single_winner_cas_provenance_on_original_compute_operation",
      preserved: ["persisted_binding", "persisted_unknown_mutation_ledger", "launch", "debit", "cvm", "allocation", "ownership"],
      consumer: "original_ClaimComputeRecovery_only",
      mutationBounds: { sub2api: 0, tencent: 0, kubernetesNodePatchMax: 1 },
      failClosed: ["zero_or_multiple_compute_candidates", "other_binding_mismatch", "ledger_drift", "provider_identity_drift", "storage_started", "cas_conflict", "unknown_readback"]
    },
