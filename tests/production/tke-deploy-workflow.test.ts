...
test("dedicated NodePool bootstrap is the only manual CreateNodePool workflow", async () => {
  ...
  assert.equal(inputs.migrate_package_taints.default, "false");
  assert.deepEqual(inputs.migrate_package_taints.options, ["false", "true"]);
  assert.equal(inputs.taint_migration_confirmation.required, false);
  assert.match(String(job.env.OPL_BASIC_COMPUTE_INSTANCE_TYPE), /migrate_package_taints.*vars\.OPL_BASIC_COMPUTE_INSTANCE_TYPE/);
  assert.match(String(job.env.OPL_PRO_COMPUTE_INSTANCE_TYPE), /migrate_package_taints.*vars\.OPL_PRO_COMPUTE_INSTANCE_TYPE/);
  ...
  assert.match(operationModeGate, /MUTATE_MISSING_POOLS/);
  assert.match(operationModeGate, /MIGRATE_PACKAGE_TAINTS/);
  assert.match(operationModeGate, /separate mutation modes/);
  ...
  assert.match(String(job.env.RUN_TENCENT_NODE_POOL_TAINT_MIGRATION), /MIGRATE_BASIC_PRO_NODEPOOL_PACKAGE_TAINTS/);
  assert.equal(job.env.RUN_TENCENT_NODE_POOL_TAINT_MIGRATION_CONFIRMATION, "${{ inputs.taint_migration_confirmation }}");
  assert.match(runs, /migrate_workspace_node_pool_taints/);
  const bootstrapAction = stepsByName(job).get("Inventory and bootstrap package NodePools");
  assert.match(String(bootstrapAction.if), /migrate_package_taints != 'true'/);
  for (const stepName of ["Inventory and select Workspace SKUs", "Revalidate selected Workspace SKUs before mutation"]) {
    assert.match(String(stepsByName(job).get(stepName).if), /migrate_package_taints != 'true'/);
  }
  ...
  assert.match(migrationReportGate, /pool\.taintsAfter/);
  assert.match(migrationReportGate, /Number\.isInteger\(pool\.estimatedNodeCount\)/);
  assert.match(migrationReportGate, /oplcloud\.cn\/package-id/);
  assert.match(migrationReportGate, /oplcloud\.cn\/workspace-id/);
  assert.match(migrationReportGate, /nodePoolId !== process\.env\.OPL_SYSTEM_COMPUTE_NODE_POOL_ID/);
  assert.match(migrationReportGate, /poolCheckStatus !== "passed"/);
  assert.match(migrationReportGate, /machineCheckStatus !== "passed"/);
