...
| Contracts | Control Plane, Fabric, Ledger, tools | `CONTRACT-DEDUP-02` | Retain the fixed Runtime port through its versioned ABI owner; the remaining slice removes the second mutable price catalog while preserving accepted snapshots. |
...
| --- | --- | --- | --- | --- | --- |
| `CONTRACT-DEDUP-02` | `next` | `P1` | Exact current price values remain duplicated between Control Plane runtime code and `opl-cloud-pricing-contract.json`; the fixed Runtime port duplicate and false environment option are closed in current source | public APIs, accepted price snapshots, security, integrity, permissions, irreversible side effects | Control Plane has one versioned runtime price catalog and accepted Workspaces retain immutable snapshots; cross-module contracts describe price schema/invariants without a second mutable catalog |
