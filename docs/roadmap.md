...
| --- | --- | --- | --- | --- | --- |
| `MVP-LOCAL-WORKSPACE-GATEWAY-01` | `next` | `P0` | Thin Console, one Control Plane Reconciler, typed Fabric stage bindings, a real `local-docker` adapter, and Sub2API-backed balance/usage/Key/debit/refund paths exist, but the portable installation does not enable the launch worker, mount a Docker authority, or select an immutable Workspace image; the backend Workspace delete command and one complete accounting/readback path are also absent | Fabric provider port + Control Plane Launch coordination + Gateway/Sub2API authority + thin Console; Ledger records only required receipts and reconciliation | On a MacBook or single-server Docker host, Console creates, reads back, opens, and deletes one OPL App/WebUI Workspace through the single Control Plane Reconciler and `local-docker`; every resource stage uses the admitted Fabric binding; balance, usage, debit, and refund remain Sub2API-authoritative; Tencent/TKE is neither exercised nor required |
...
The immediate evidence gaps are one complete local Console-to-Workspace path and
one authoritative Gateway accounting flow on that same path. Self-service onboarding/payment, refined
