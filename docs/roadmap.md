...
| --- | --- | --- | --- | --- | --- |
| `SECURITY-S4-LOW-FINDINGS` | `candidate` | `P2` | The release Environment gap is resolved within S1, and the login-CSRF slice is absorbed through PR `#283`: login requires JSON media type, explicit cross-origin `Origin`/`Referer` is rejected before session material, same-origin Console JSON remains accepted, and headerless non-browser JSON remains compatible. Five independent low findings remain: support-ticket server fields, mutable build inputs, release hashes/attestation, HTTP body bounds, and high-cardinality login-rate state; none is admitted for source mutation by this row | Each remaining real owning module or release workflow; no omnibus security package or shared policy layer | Each remaining finding receives caller, boundary, precondition, counterevidence, exact write set, focused test, and residual-risk triage; only independently valuable, zero-overlap slices advance to `next` or `planned`. The absorbed login control remains covered without being generalized into a broad browser-security completion claim |
