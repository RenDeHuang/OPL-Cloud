...
| `PRODUCT-RELEASE-01` | `next` | `P1` | Workflow source now validates `compose.local-workspace.yaml`, records it in the release manifest, uploads the exact four-asset set, and tests release readback. No OPL Cloud tag, GitHub Release, GHCR package, or clean-host installation evidence exists, so the protected release and installation gap remains open | Protected Cloud release execution and immutable product artifacts; concrete installation configuration remains instance-owned | One immutable `v0.x.y` release from exact Cloud `main` publishes the base Compose file, `compose.local-workspace.yaml`, environment template, manifest, and GHCR multi-architecture digest; manifest, tag, target SHA, asset set, and digest read back exactly; a clean Docker host using only those release assets plus its own Sub2API/Workspace configuration completes the admitted install path |
...
| `MODULE-COHESION-01` | `next` | `P1` | Control Plane Launch is now split into one provider-neutral Reconciler and focused stage files, Fabric `service.go` was reduced substantially, Tencent compute ownership has one adapter-private core, and the targeted operator identity-evidence and pool-head replay paths use narrow exact owner lookups. Provider Facts Phase A is closed: the generic Service delegates provider interpretation to the selected adapter, Tencent mapping lives in its adapter, and local-Docker has compute/storage/attachment/Runtime parity. Fabric still retains a large mixed facade and other Tencent validation/projection knowledge outside this provider-facts path | One owning Go module per change | Focused capability files preserve public contracts, state behavior, and full tests without a shared domain/policy package; generic Fabric code has no provider-specific validation or identity inference |
| `INSTANCE-PROVIDER-ACCEPTANCE-MIGRATION-01` | `next` | `P1` | Provider Facts Phase A no longer interprets Tencent `InstanceType`, `providerData`, CVM/CBS, NodePool, or `costTags` in generic Fabric Service code; the typed batch route delegates read-only mapping to adapters. The separate Control Plane provider-acceptance route, legacy Fabric client DTOs, persisted projections, and Instance callers remain on the Tencent/TKE shape and are not cut over | Reusable provider-neutral acceptance contract in Cloud + Tencent/TKE implementation/profile in Fabric adapter and `opl-instance-medopl` | Real callers move to a provider-neutral acceptance/readback contract; Tencent offer/profile constants and resource interpretation leave Control Plane; retained historical fields are read-only migration evidence or are explicitly migrated; normal Launch remains unchanged and no second Reconciler appears |
...
| --- | --- | --- | --- | --- | --- |
| `SECURITY-S0-GITHUB-CONTROLS` | `immediate` | `P1` | The disclosure policy, PR dependency-diff job, and expanded dependency update coverage have not reached canonical `main`. Dependency Review has succeeded at an exact candidate SHA and its exact context is now required alongside strict `validate`; Actions sources are allowlisted and SHA-pinned. Secret validity checks remain disabled | `SECURITY.md`, narrow `.github` configuration, and GitHub repository settings; no product runtime mutation | Policy and Dependabot configuration reach `main`; required `validate` and `dependency-review` succeed there; GitHub settings and open-alert counts receive exact API readback; secret validity becomes enabled or is confirmed unavailable by its external owner |
| `SECURITY-S1-RELEASE-PRIVILEGE` | `planned` | `P1` | Medium finding: the release job executes repository and dependency code while already holding `contents:write`, `packages:write`, checkout credentials, and `GH_TOKEN`; the same path lacks a dedicated protected `cloud-release` Environment. Immutable Releases are disabled and no tag ruleset exists | Cloud release workflow only; build is read-only, publish alone receives write authority and a release Environment; no instance deployment | Focused workflow tests prove read-only build and artifact handoff to a minimal publish job; `cloud-release` is protected and bound only to publish; existing four-asset manifest semantics remain; no real release is triggered; immutable-release and `v*` tag policy are enabled only after compatibility readback |
| `SECURITY-S2-FABRIC-CAPABILITY` | `planned` | `P1` | Medium finding: one Fabric bearer token authorizes cross-tenant resource mutation, restore, and runner claim without final account/Workspace/resource capability verification | Fabric owns caller identities and narrow operation capabilities; Control Plane and runners receive only their required scope; no shared auth framework or second provider contract | Separate identities and short-lived operation-bound capability data are verified at the Fabric service boundary; cross-tenant and wrong-operation tests fail before provider or operation-store mutation |
| `SECURITY-S3-TRUSTED-LOCAL-IMAGE` | `planned` | `P1` | Medium finding: Local Docker accepts any registry image with a digest and then mounts Workspace data and a Gateway Secret | Local Docker adapter owns one narrow trusted repository/release-manifest allowlist; generic Fabric and other providers remain unchanged | An unapproved repository digest is rejected before pull/run or Secret mount; an approved immutable release image succeeds; focused tests prove no provider mutation on rejection |
| `SECURITY-S4-LOW-FINDINGS` | `candidate` | `P2` | Seven low findings cover support-ticket server fields, release Environment, mutable build inputs, login CSRF, release hashes/attestation, HTTP body bounds, and high-cardinality login-rate state. Only the release Environment is already admitted into S1 | Each real owning module or release workflow; no omnibus security package or shared policy layer | Each finding receives caller, boundary, precondition, counterevidence, exact write set, focused test, and residual-risk triage; only independently valuable, zero-overlap slices advance to `next` or `planned` |
| `SECURITY-CODEQL-TRIAGE` | `immediate` | `P1` | CodeQL default setup reports 15 open high-security-severity alerts from two rule families across product source, tools, and tests; workflow success alone does not establish exploitability | `SECURITY.md` boundary plus each cited source owner; alerts remain GitHub evidence, not a public issue dump or duplicate status ledger | Every alert is grouped by common dataflow and classified with source, sink, supported boundary, preconditions, counterevidence, owner, and `confirmed`/`needs_review`/`not_actionable`; only validated fixes close through code plus rerun readback |
