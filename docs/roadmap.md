...
| --- | --- | --- | --- | --- | --- |
| `SECURITY-S0-GITHUB-CONTROLS` | `immediate` | `P1` | The repository lacked a disclosure policy, PR dependency-diff gate, and dependency update coverage beyond Actions. GitHub controls are partly enabled, but Dependency Review has not yet produced a canonical PR check and secret validity checks remain disabled | `SECURITY.md`, narrow `.github` configuration, and GitHub repository settings; no product runtime mutation | Policy and Dependabot configuration reach `main`; Dependency Review succeeds on a real PR before its exact context is added to strict required checks; GitHub settings and open-alert counts receive exact API readback |
