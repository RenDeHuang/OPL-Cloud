...
| `CONSOLE-UI-01` | `in_review` | `P3` | PR #218 refreshes the public and authenticated experience but is not yet on fresh canonical `main` | Console UI presentation and focused behavior tests | After the MVP path is usable, fresh-main replay passes type, lint, focused UI, production CSP, responsive browser, accessibility, and canonical readback without claiming a functional feature |
...
| `SIMPLIFY-CONSOLE-CSS-01` | `later` | `P3` | Multiple retained visual generations after PR #218 | medium | After the MVP path is usable, preserve current desktop/mobile behavior and Workspace interaction states while consolidating styles |
