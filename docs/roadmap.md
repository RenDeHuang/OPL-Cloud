...
| `SECURITY-SCAN-REMEDIATION-02` | `in_review` | `P1` | PRs `#307` and `#308` absorbed the first eight findings and the follow-up terminalization capability fix. Sealed Standard scan `55437d10-456a-41ae-b39c-c7e4f0cdbd81` against canonical `5fc1e5fba29837c6fb2215c427992de966b6a5e5` no longer reproduced that omission but reported five new findings: Control Plane body-limit bypass, Fabric Runtime password disclosure, zero-caller Fabric sync HTTP writes, Ledger pre-capability owner-query exhaustion, and unbounded renewal command history. The current candidate fixes all five at the source/test layer; they remain open pending absorption and a fresh sealed scan. Instance credential adoption and GitHub platform immutability remain external-owner evidence | Cloud source, contracts, and tests; GitHub settings and Instance/runtime evidence remain outside this repository's write authority | Fresh canonical `main` readback plus a sealed scan shows no remaining source finding; platform, Instance adoption, and production claims remain separately owner-authoritative |
