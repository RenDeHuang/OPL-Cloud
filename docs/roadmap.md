...
| ID | State | Priority | Current gap | Owner | Acceptance |
| --- | --- | --- | --- | --- | --- |
| `CONSOLE-LAUNCH-CONSISTENCY-01` | `next` | `P1` | Catalog/Console show Basic and Pro while controlled Pilot admission accepts only Basic; Console rejects exact-balance purchase while the server accepts it | Control Plane product policy and Console projection | One product decision is reflected by catalog, quote, Console, server admission, and focused boundary tests |
