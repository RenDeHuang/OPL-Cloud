...
| `PRODUCT-RELEASE-01` | `next` | `P1` | Workflow source now preserves the exact four-asset manifest contract and separates read-only OCI build/validation from protected publication; the `cloud-release` Environment allows protected branches only. No OPL Cloud tag, GitHub Release, GHCR package, hosted release-workflow execution, or clean-host installation evidence exists, so the release and installation gap remains open | Protected Cloud release execution and immutable product artifacts; concrete installation configuration remains instance-owned | One immutable `v0.x.y` release from exact Cloud `main` publishes the base Compose file, `compose.local-workspace.yaml`, environment template, manifest, and GHCR multi-architecture digest; manifest, tag, target SHA, asset set, digest, Environment binding, and minimal publish permissions read back exactly; a clean Docker host using only those release assets plus its own Sub2API/Workspace configuration completes the admitted install path |
...
| --- | --- | --- | --- | --- | --- |
| `SECURITY-S1-RELEASE-PRIVILEGE` | `in_review` | `P1` | The candidate release workflow closes the medium finding structurally: repository/dependency code runs only in a read-only build job without checkout credentials or `GH_TOKEN`; a digest-checked artifact crosses into a publish-only job that runs no repository code or external Actions and alone receives write permissions. The protected-branch-only `cloud-release` Environment is live. Canonical absorption and exact-head hosted checks remain; no release has run | Cloud release workflow and focused contract test only; no instance deployment or real publication | Exact candidate SHA passes focused workflow contracts, Action pinning, `actionlint`, complete local repository gates, and hosted required checks; canonical `main` preserves the split and Environment binding. First real release, tag policy, and immutable-release compatibility remain under `PRODUCT-RELEASE-01`, not this finding |
...
| `SECURITY-S4-LOW-FINDINGS` | `candidate` | `P2` | The release Environment gap is resolved within S1. Six remaining low findings cover support-ticket server fields, mutable build inputs, login CSRF, release hashes/attestation, HTTP body bounds, and high-cardinality login-rate state; none is admitted for source mutation by this row | Each real owning module or release workflow; no omnibus security package or shared policy layer | Each finding receives caller, boundary, precondition, counterevidence, exact write set, focused test, and residual-risk triage; only independently valuable, zero-overlap slices advance to `next` or `planned` |
