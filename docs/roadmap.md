...
| `SECURITY-SCAN-REMEDIATION-02` | `in_review` | `P1` | Canonical scan revision `b7217daddf1520d7f442cea5b8dba2c6df636cdf` reported eight source findings. The candidate closes the Ledger/Fabric capability gaps, portable image pinning, Workspace destination admission, and Control Plane resource bounds with focused tests; it still needs canonical absorption and a fresh scan. GitHub platform immutability remains external-owner evidence | Cloud source, contracts, and tests; GitHub settings and Instance/runtime evidence remain outside this repository's write authority | Fresh canonical `main` readback plus a new scan shows the source findings no longer reproduce; platform and production claims remain separately owner-authoritative |
