...
S1 and `SECURITY-CODEQL-TRIAGE-01` are no longer active plan rows: their terminal
source and classification evidence belongs in [status.md](./status.md). The 15
CodeQL alerts remain open because no dismissal, fix, or settings mutation was
authorized or performed; that open disposition is not an admitted fix lane.

| ID | Class | Priority | Current gap | Owner boundary | Acceptance |
| --- | --- | --- | --- | --- | --- |
| `SECURITY-S2-FABRIC-CAPABILITY` | `next` | `P1` | Medium finding: one Fabric bearer token authorizes cross-tenant resource mutation, restore, and runner claim without final account/Workspace/resource capability verification. An `ACTIVE` official receipt admits the implementation lane; the capability boundary remains open | Fabric owns caller identities and narrow operation capabilities; Control Plane and runners receive only their required scope; no shared auth framework or second provider contract | Separate identities and short-lived operation-bound capability data are verified at the Fabric service boundary; cross-tenant and wrong-operation tests fail before provider or operation-store mutation |
| `SECURITY-S3-TRUSTED-LOCAL-IMAGE` | `next` | `P1` | Medium finding admitted for implementation: Local Docker accepts any registry image with a digest and then mounts Workspace data and a Gateway Secret. Admission does not establish an implemented control or product readiness | Local Docker adapter owns one narrow trusted repository/release-manifest allowlist; generic Fabric and other providers remain unchanged | An unapproved repository digest is rejected before pull/run or Secret mount; an approved immutable release image succeeds; focused tests prove no provider mutation on rejection |
