...
| --- | --- | --- | --- | --- | --- |
| `MVP-LOCAL-WORKSPACE-GATEWAY-01` | `next` | `P0` | Thin Console, one Control Plane Reconciler, typed Fabric stage bindings, a real `local-docker` adapter, Sub2API-backed balance/usage/Key/debit/refund paths, and an explicit local-Workspace Compose profile exist. The profile enables the Launch worker, mounts Docker authority only into Fabric, and requires an immutable Workspace image; a real smoke reached the host Docker Engine but stopped at the required external Sub2API authentication boundary. One complete Console create/readback/open/delete path with authoritative accounting remains unproven | Fabric provider port + Control Plane Launch coordination + Gateway/Sub2API authority + thin Console; Ledger records only required receipts and reconciliation | On a MacBook or single-server Docker host, Console creates, reads back, opens, and deletes one OPL App/WebUI Workspace through the single Control Plane Reconciler and `local-docker`; every resource stage uses the admitted Fabric binding; balance, usage, debit, and refund remain Sub2API-authoritative; Tencent/TKE is neither exercised nor required |
| `PRODUCT-RELEASE-01` | `next` | `P1` | GitHub has no OPL Cloud tag or Release. The product release workflow publishes the base Compose file, environment template, manifest, and multi-architecture image, but omits the explicit local-Workspace override that now exists in source; no clean host has installed an exact release and completed the product path | Reusable release workflow and immutable product artifacts in Cloud; concrete installation configuration remains instance-owned | One immutable `v0.x.y` release from exact Cloud `main` publishes the base Compose file, local-Workspace override, environment template, manifest, and GHCR multi-architecture digest; manifest, tag, target SHA, asset set, and digest read back exactly; a clean Docker host using only those release assets plus its own Sub2API/Workspace configuration completes the admitted install path |
...
| `MODULE-COHESION-01` | `next` | `P1` | Control Plane Launch is now split into one provider-neutral Reconciler and focused stage files, and Fabric `service.go` was reduced substantially; Fabric still retains a large mixed facade, remaining Tencent validation/projection knowledge, operation-list scans in operator extensions, dead launch/recovery shells, and duplicate ownership logic | One owning Go module per change | Focused capability files preserve public contracts, state behavior, and full tests without a shared domain/policy package; generic Fabric code has no provider-specific validation or identity inference, active readback uses exact owner keys, and no dead or duplicate mutation path remains |
...
| `SIMPLIFY-FABRIC-DEAD-LAUNCH-SHELL-01` | `next` | `P1` | Typed Launch cutover removed the recovery proof/claim HTTP entrypoints, but their Service/provider/store mutation shell remains; five legacy resource inputs also retain unassigned `LaunchBinding` branches and an orphan binding-readback method | medium | Fresh source and external-consumer scans prove zero callers; delete the dead DTO, Service, provider, store, input, readback, and test paths while preserving active identity evidence, pool-head terminalization, typed stage binding persistence, historical migrations/data, and the real local-Docker gate |
| `SIMPLIFY-FABRIC-TENCENT-OWNERSHIP-01` | `next` | `P1` | Typed Workspace Launch and `TagComputeMachine` each implement Tencent CVM/node ownership mutation and replay readback | medium | One adapter-private ownership core serves both callers with the same deterministic child operation, exact readback, and idempotency semantics; it has no dependency on recovery DTOs or a second operation store |
