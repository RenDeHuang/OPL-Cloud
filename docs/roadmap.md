...
| `PRODUCT-RELEASE-01` | `next` | `P1` | Current publication is `v0.1.7`: hosted run `31879240411` published five assets and a verified `linux/amd64` + `linux/arm64` GHCR index from product SHA `a59bde68397528186a5220f73195fa1f3eda311b`. Eight Releases in roughly 49 hours exposed a missing semantic admission boundary; Product policy now requires one bounded `READY` release unit, a prepared consumer, and proof that the preceding Release cannot satisfy it. The workflow is create-only, but GitHub platform immutability remains disabled (`immutable=false`, no tag ruleset). Remaining functional gaps are clean Docker-host installation and the end-to-end local-Workspace path with real external Sub2API credentials/readback | Cloud owns portable artifacts, release admission, and the release workflow; installation environment and production qualification remain instance/operator-owned | A clean Docker host uses one admitted exact Release plus its own Sub2API/Workspace configuration to complete Console create/readback/open/delete with authoritative resource and accounting readback; tag/release immutability is either platform-enforced or explicitly accepted as a documented residual risk; each successor Release has a distinct bounded unit, exact Cloud SHA/digest, prepared consumer, and predecessor gap rather than serving as a debug checkpoint |
