...
| Ledger | Control Plane, qualification tooling | `EVIDENCE-INDEX-01` | Record purchase/deletion receipts, accepted price snapshots, and the append-only cross-surface Evidence Index; do not price, mutate wallet balance, copy service state, or authorize Workspace actions. |
...
| Contracts | Control Plane, Fabric, Ledger, tools | No independent current lane | Retain pricing schemas and immutable accepted snapshots without a second mutable catalog; retain the fixed Workspace Runtime ABI without an Instance override. |
...
| --- | --- | --- |
| Basic/Pro visibility and exact-balance admission disagree | Closed in source; the admitted offer set is Basic/Pro and both Console and Control Plane accept an exact available-balance match | `CONSOLE-LAUNCH-CONSISTENCY-01` |
| Customer renewal authorization exists in source, but reactivation and live renewal evidence remain absent | Keep the source-level authorization closed; implement explicit reactivation and qualify the exactly-once renewal path against its live owners | `WORKSPACE-RENEWAL-REACTIVATION-01` |
...
| --- | --- | --- | --- | --- | --- |
| `MVP-LOCAL-WORKSPACE-GATEWAY-01` | `next` | `P0` | No one release-qualified clean-host run binds external identity/wallet/usage, Workspace create/readback/open/restart, permanent no-refund Delete, final resource/Key absence and the deletion Receipt to one exact Candidate. The qualification runner is source-aligned to the current Workspace request and Delete contracts, but has no successful exact-current Candidate run | Control Plane owns Launch/Delete and commercial coordination; Fabric owns Local-Docker resources; Sub2API owns wallet/Key/usage; Ledger owns purchase/deletion receipts; Console projects | On one supported clean Linux Docker host, the same Candidate completes login, quote/debit, create/readback/open, restart, permanent Delete, final resource/Key absence, unchanged post-debit wallet history, and exactly one `workspace.deleted.v1` receipt. Delete performs zero wallet or refund mutation, and the qualification tool/tests match that contract |
| `CONSOLE-LAUNCH-CONSISTENCY-01` | `closed` | `P0` | The Control Plane admits the Basic/Pro offer set returned by the provider catalog, rejects unknown or unavailable packages before persistence, debit or provider mutation, and the Console uses the same `balance >= quote` boundary as the server | Control Plane offer/admission policy + Console projection | Focused Control Plane tests cover Basic/Pro, stopped or invalid global admission, unavailable packages and exact-balance launch; focused Console tests cover below, equal and above-quote balances |
...
| `WORKSPACE-RENEWAL-REACTIVATION-01` | `planned` | `P1` | Customer Workspace enable/disable, durable Launch intent, activation continuity and Console authoritative readback are implemented in source and local PostgreSQL owner tests. An expired customer still has no reactivation command, and no exact Candidate has live Sub2API Debit, Fabric renewal/readback, Ledger Receipt and Instance evidence for the full path | Control Plane settlement + Sub2API adapter + Fabric renewal/readback + Ledger receipts | Add an explicit authorized reactivation command, then qualify one exact Candidate through customer authorization, one Workspace-period Debit, Provider renewal/readback, Receipt and failure recovery without a second wallet or implicit reactivation |
