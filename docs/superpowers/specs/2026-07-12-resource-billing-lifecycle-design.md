...
-> 写入并重新读取 CVM 实例名与 Kubernetes Node Label
...
- 数据库 ownership、CVM 实例名和 Node Label 必须指向同一个 `resource_id`；
...
认领后，Fabric 把 CVM 实例名写为 `resource_id`，并给 Kubernetes Node Label 写入
`resource_id`、`account_id` 和 `workspace_id`，随后重新读取验证。实例名和标签用于运维和漂移检查，
...
- ownership 已 claimed，但实例名、Label 或 Provider 身份尚未通过复核；
...
- ownership、CVM 实例名和 Node Label 不一致时禁止扣费并进入隔离；
