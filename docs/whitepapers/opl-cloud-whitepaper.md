...
OPL Cloud 已经进入产品落地阶段。当前实现已形成独立的 Console、Control Plane、Fabric 和 Ledger 服务边界，并发布了可移植产品制品；一条完整的本地 Workspace 与真实 Gateway 贯通路径仍在持续完成和验证。产品愿景、工程落地与公开可用性因此由各自证据清楚表达。
...
Cloud 产品持续建设和发布，具体能力则按照项目的工作场景与责任边界接入。

App 可以有不同界面实现，产品定义始终保持统一。当前 AionUI 主线界面与借鉴 DeepSeek
Harness 的候选界面共同遵循 App 产品合同和 Framework 状态/动作接口；界面变化不会改变
Cloud 的账号、资源、账单和证据责任。Framework 的运行状态视图帮助 App 呈现工作进展，
OPL Cloud Console 则面向账号、策略与管理场景，两者各自承担清楚的产品责任。
...
Cloud Console 负责账号、策略、批准、额度和云端 Workspace 管理；本机 App 负责专业工作
体验；Framework 负责组合经过准入的运行能力，并向工作界面提供清楚的状态与动作。
不同界面共享同一份产品能力、动作边界和结果证据。用户看到的是同一套 OPL 工作语言，
产品内部的界面、服务和资源实现则可以独立演进。
