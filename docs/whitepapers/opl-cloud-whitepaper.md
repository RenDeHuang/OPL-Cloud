...
核心判断：云端不应迫使用户离开正在推进的成果，也不应把基础设施复杂度转嫁给用户。OPL Cloud 让工作从本机自然延伸到在线 Workspace、私有数据、远端资源、协作和 Agent 服务，同时让权限、成本、证据与专业判断各归其位。

## 定位摘要

OPL Cloud 是 One Person Lab 面向复杂知识工作的云端产品。用户继续围绕问题、材料、阶段、产物和审阅推进工作；系统在背后连接 AI、在线工作空间、账号策略、数据与计算资源、Agent 服务和可复查证据。

它不是把桌面软件搬到服务器，也不是把云资源目录包装成一个控制台。OPL Cloud 的产品价值，是让长期工作在环境变化后仍保持上下文连续，让资源使用受到清楚约束，让每份结果能回到它的来源与责任方。
...
账号控制、运行证据、AI 接入和 Agent 服务等名称，描述的是责任和用户
问题，而不是某个固定的源码目录或插件数量。同一个能力域可以由不同产品
共同完成；真正的权威仍然只在明确的产品、服务或领域负责人手中。

因此，Cloud 的 Console 是账号、策略、批准、额度和云端 Workspace 的控制
面；它不是本机 App 的全部界面，也不拥有 Framework 的运行时。Framework
负责把经过准入的能力组合成 Host，并把允许给工作界面的能力投影出去；
App 可以使用不同的界面载体，但它们共享同一份产品能力、动作边界和结果
证据。用户看到的是同一套 OPL 工作语言，产品内部则可以独立替换界面、
服务或资源实现，而不复制权威和状态。

这种分离也让版本更容易理解：服务、界面载体和可复用能力可以按各自真实
的发布节奏演进，产品配置只选择经过验证的组合，不把所有内部组件暴露成
一个难以管理的任意组合矩阵。
...
本机 OPL App 适合个人控制、敏感材料和日常编辑；在线 OPL Workspace 适合远程访问、协作和托管执行。两者不是两套互不相干的产品语言，而是同一工作模型的不同载体。
...
最重要的设计不是把所有事实集中到一个数据库，而是让界面连接各自可靠的来源。用户看到一致体验，背后的资源、服务、证据和专业判断仍由最合适的责任方维护。
...
四条路径共享同一组设计判断：从成果出发、按需扩展、关键动作授权、责任各归其位。
...
本文描述的是产品选择，而不是对某项能力当前可用性的承诺。读者可以用它判断 OPL Cloud 是否对复杂知识工作的真实矛盾做出了完整、专业和一致的回答。
